...
while ($row = mysqli_fetch_array($result));
	...
	?>
		...
		<?php
	include("include/block_parameter2.php");
	?>
...
if($_GET["manufact"])
 {
  ...
  $end_price = (int)$_GET["end_price"];
   ...
   if (!empty($check_manufact) OR !empty($end_price))
   {
    ...
    if (!empty($end_price)) $query_cost = "AND CostFilter BETWEEN $start_price AND $end_price"; //поиск оборудования в данном диапазоне
    
   }
      
   $result = mysqli_query($db,  "SELECT * FROM filter WHERE visible='1' $query_manufact $query_cost ORDER BY idFilter");               
