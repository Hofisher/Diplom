				...
				<ul class="nav">
					...
					<li><a href=".php">Ещё <i> </i></a>
						<ul>
							...
							<li><a href="manufacture.php">Производители</a></li>					</ul>
					</li>
				</ul>
...
while ($row = mysqli_fetch_array($result));
	...
	?>
				...
				<ul id="reactor-grid">
   		      	          <?php
                          if (isset($_GET['idCentrifug']))
                          {
                            echo $_GET['idCentrifug']."<br>";
                            
                          }

   $result = mysqli_query($db,  "SELECT * FROM centrifugi WHERE idCentrifug='$idencefic'");
...
$heidht = 200;
		 ...
		 <a href="Centrifug_Description.php"><img class="img-responsive " src="'.$img_path.'" width="'.$width.'" heidht="'.$heidht.'" /></a> 
		 </div>
		<div class="shoe-in"> <a href= "Centrifug_Description.php?idCentrifug='.$row["idCentrifug"].'&TypeApparatus='.$row["mini_description"].'""> '.$row["NameCentr"].'</a> </p>
		 <a class="add-cart-style-grid"> </a>
		 <p class="style-price-grid"><strong>'.$row["cost"].'</strong>руб.</p>
		 <h6><a href="Centrifug_Description.php"class="store"><div class="mini-description">'.$row["mini_description"].'</div>
          			 </a></h6>
			   </div>
			   			  </div>  
		 </li>
...
?>     
  <?php
                          if (isset($_POST['idCentrifug']))
                          {
                            echo $_POST['idCentrifug']."<br>";
