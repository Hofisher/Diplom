...
<<?php
    ...
    <form method="GET" action="filter_algoritm.php" id="form_series">
        <?php


        $result = mysqli_query($db,"select * from filter as c 
group by SeriesFilter;");
        ...
        if (mysqli_num_rows($result)>0)
        {
            ...
            do
            {
                ...
                echo '  
							...
							value="'.$row["SeriesFilter"].'" id="checkSeries'.$row["idFilter"].'"/>
							<label for="checkSeries'.$row["idFilter"].'">'.$row["SeriesFilter"].' '.$row["NameFilter"].'</label></li>
							</ul> 
								';
            }
...
case
COLUMN_NAME when'CostFilter' then 'Стоимость'
when 'Weight' then 'Вес'
when 'Height' then 'Высота'
when 'TempSredyMIN' then 'Минимальная температура среды'
when 'TempSredyMAX' then 'Максимальная температура среды'
when 'VolumeNominal' then 'Номинальный объем'
when 'DrivePower' then 'Мощность двигателя'
when 'Pressure' then 'Давление'
else 'Площадь фильтрации'
end as CN
FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='filter' and COLUMN_NAME in ('CostFilter' ,'Weight','Height','TempSredyMIN','TempSredyMAX','VolumeNominal','DrivePower','Pressure', 'FiltrationArea')
) a;");
            $result2 = mysqli_query($db,"SELECT distinct COLUMN_NAME
FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='filter' and 
COLUMN_NAME in ('CostFilter' ,'Weight','Height','TempSredyMIN','TempSredyMAX','VolumeNominal','DrivePower','Pressure', 'FiltrationArea')");
