...
<ul class="checkbox-brand">
	...
	$result = mysqli_query($db,"SELECT * FROM typeapp WHERE TypeApparatus='сборники'");
    if (mysqli_num_rows($result)>0)
    {
     $row = mysqli_fetch_array($result);
        ...
        {
            echo '
            <p><li><a href="view_catSborniki.php?cat='.$row["NameApparatus"].'&TypeApparatus='.$row["TypeApparatus"].'">'.$row["NameApparatus"].'</a></li></p>
...
while ($row = mysqli_fetch_array($result));
	...
	?>
		...
		<?php
	include("include/block_parameter3.php");
	?>
...
if($_GET["manufact"])
 {
  $check_manufact = implode(',',$_GET["manufact"]);  
  }
  $start_price = (int)$_GET["start_price"];//пропускать только цифры
  $end_price = (int)$_GET["end_price"];

   if (!empty($check_manufact) OR !empty($end_price))
   {
    
    if (!empty($check_manufact)) $query_manufact = " AND manufacture IN ($check_manufact)";
    if (!empty($end_price)) $query_cost = "AND cost BETWEEN $start_price AND $end_price"; //поиск оборудования в данном диапазоне
    
   }
      
   $result = mysqli_query($db,  "SELECT * FROM sborniki WHERE visible='1' $query_manufact $query_cost ORDER BY idSbornik");               
    ...
    if (mysqli_num_rows($result)>0)
    {
     $row = mysqli_fetch_array($result);
        do 
        {
if ($row["photo_sbornik"] != "" && file_exists("./upload_images/".$row["photo_sbornik"]))  //проверка "указано ли в базе в поле реакторов изображения" если поле не равняется пустоте и файл существует ,то true (точку перед размещением сайта в интернет ./web нужно убрать)
{//функция которая уменьшает размер изображения
$img_path = './upload_images/'.$row["photo_sbornik"];
$max_width = 350;
$max_height = 350;
...
$heidht = 200;
		 ...
		 <a href="Sbornik_Description.php"><img class="img-responsive " src="'.$img_path.'" width="'.$width.'" heidht="'.$heidht.'" /></a>
		 </div>
		<div class="shoe-in"> <a href= "Sbornik_Description.php?idSbornik='.$row["idSbornik"].'&TypeApparatus='.$row["mini_description"].'""> '.$row["NameSbornik"].'</a> </p>
		 <a class="add-cart-style-grid"> </a>
		 <p class="style-price-grid"><strong>'.$row["cost"].'</strong>руб.</p>
		 <h6><a href="Sbornik_Description.php"class="store"><div class="mini-description">'.$row["mini_description"].'</div>
			 </a></h6>
			   </div>
			   			  </div> 
		 </li>
