...
if($_GET["manufact"])
 {
  ...
  $end_price = (int)$_GET["end_price"];
   ...
   $result = mysqli_query($db,  "SELECT * FROM sushilki WHERE visible='1' $query_manufact $query_cost ORDER BY idSushilki");               
...
$heidht = 200;
		 ...
		 <a href="Sushilki_Description.php"><img class="img-responsive " src="'.$img_path.'" width="'.$width.'" heidht="'.$heidht.'" /></a>
		 </div>
		<div class="shoe-in"> <a href= "Sushilki_Description.php?idSushilki='.$row["idSushilki"].'&TypeApparatus='.$row["mini_description"].'""> '.$row["NameSush"].'</a> </p>
		 <a class="add-cart-style-grid"> </a>
		 <p class="style-price-grid"><strong>'.$row["cost"].'</strong>руб.</p>
		 <h6><a href="Sushilki_Description.php"class="store"><div class="mini-description">'.$row["mini_description"].'</div>
			 </a></h6>
			   </div>
			   			  </div> 
		 </li>
