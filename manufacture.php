...
<!--header-->
				...
				<ul class="nav">
					...
					<li><a href="product.php">Ещё <i> </i></a>
						<ul>
							...
							<li><a href="register.php">Производители</a></li>	
				</ul>
					</li>
				</ul>
	...
	<!---->	
		...
		<h2>Производство фармацевтических субстанций</h2>
			
				<ul id="reactor-grid">
			
		      	          <?php
	$result = mysqli_query($db,"SELECT * FROM pharm_industry");
    if (mysqli_num_rows($result)>0)
    {
     $row = mysqli_fetch_array($result);
        do 
        {
if ($row["technologicalShema"] != "" && file_exists("./upload_images/".$row["technologicalShema"])) //проверка "указано ли в базе в поле реакторов изображения" если поле не равняется пустоте и файл существует ,то true (точку перед размещением сайта в интернет ./web нужно убрать)
{//функция которая уменьшает размер изображения
$img_path = './upload_images/'.$row["technologicalShema"];
$max_width = 600;
$max_height = 200;
 list($width, $heidht) = getimagesize($img_path);$ratioh = $max_height/$heidht;
...
$heidht = 200;
		...
		<div class="shoe-in"> <a href= ""> '.$row["NameIndustry"].'</a> </p>
		 <a class="add-cart-style-grid"> </a>
				 <h6><a href="single.php"class="store"><div class="Description">'.$row["Description"].'</div>
			 </a></h6>
			   </div>
			   			  </div>  
		 </li>
	...
	<div class="col-md-3 footer-left">
		...
		<div class="col-md-4 footer-left left-footer">
			...
			<div class="letter">
				...
				<a href="register.php" class="sign">SIGN UP AND GET MORE</a>
