...
if($_GET["manufact"])
 {
  ...
  $end_price = (int)$_GET["end_price"];
   ...
   $result = mysqli_query($db,  "SELECT * FROM centrifugi WHERE visible='1' $query_manufact $query_cost ORDER BY idCentrifug");               
...
$heidht = 200;
		 ...
		 <a href="Centrifugi_Description.php"><img class="img-responsive " src="'.$img_path.'" width="'.$width.'" heidht="'.$heidht.'" /></a>
		 </div>
		<div class="shoe-in"> <a href= "Centrifugi_Description.php?idCentrifug='.$row["idCentrifug"].'&TypeApparatus='.$row["mini_description"].'""> '.$row["NameCentr"].'</a> </p>
		 <a class="add-cart-style-grid"> </a>
		 <p class="style-price-grid"><strong>'.$row["cost"].'</strong>руб.</p>
		 <h6><a href="Centrifugi_Description.php"class="store"><div class="mini-description">'.$row["mini_description"].'</div>
			 </a></h6>
			   </div>
			   			  </div> 
		 </li>
